...
import com.github.kischang.arduino.uploader.ArduinoUploader.BootloaderProgrammers.Protocols.AVR109.Avr109BootloaderProgrammer;
import com.github.kischang.arduino.uploader.ArduinoUploader.BootloaderProgrammers.Protocols.STK500v1.Stk500V1BootloaderProgrammer;
import com.github.kischang.arduino.uploader.ArduinoUploader.BootloaderProgrammers.Protocols.STK500v2.Stk500V2BootloaderProgrammer;
import com.github.kischang.arduino.uploader.ArduinoUploader.BootloaderProgrammers.ResetBehavior.IResetBehavior;
import com.github.kischang.arduino.uploader.ArduinoUploader.BootloaderProgrammers.ResetBehavior.ResetThrough1200BpsBehavior;
import com.github.kischang.arduino.uploader.ArduinoUploader.BootloaderProgrammers.ResetBehavior.ResetThroughTogglingDtrBehavior;
import com.github.kischang.arduino.uploader.ArduinoUploader.BootloaderProgrammers.ResetBehavior.ResetThroughTogglingDtrRtsBehavior;
import com.github.kischang.arduino.uploader.ArduinoUploader.BootloaderProgrammers.SerialPortConfig;
import com.github.kischang.arduino.uploader.ArduinoUploader.Config.Arduino;
import com.github.kischang.arduino.uploader.ArduinoUploader.Config.Configuration;
import com.github.kischang.arduino.uploader.ArduinoUploader.Config.McuIdentifier;
import com.github.kischang.arduino.uploader.ArduinoUploader.Config.Protocol;
...
import com.github.kischang.arduino.uploader.ArduinoUploader.Config.*;
import com.github.kischang.arduino.uploader.ArduinoUploader.Hardware.*;
import IntelHexFormatReader.*;
import IntelHexFormatReader.Model.*;
import com.github.kischang.arduino.uploader.HexFileUtils;
...
public class ArduinoSketchUploader<E extends ISerialPortStream> {
	...
	private static Logger Logger = LoggerFactory.getLogger(ArduinoSketchUploader.class);
	...
	public ArduinoSketchUploader(ArduinoSketchUploaderOptions options, Logger logger) {
	...
	public ArduinoSketchUploader(ArduinoSketchUploaderOptions options, Logger logger,
		...
		if (inferedClass == null) {
			try {
				...
				if (getLogger() != null)
					getLogger().info("Mus created as anonymous implementation (new Generic<Integer>() \\{\\};)...", e);
				e.printStackTrace();
	...
	public ArduinoSketchUploader(Class<E> clazz, ArduinoSketchUploaderOptions options, Logger logger) {
	...
	public final void UploadSketch(Iterable<String> hexFileContents, String[] allPortNames) {
		try {
			...
			if (StringHelper.isNullOrWhiteSpace(serialPortName) && portSingleOrDefault != null) {
				if (getLogger() != null)
					getLogger().info("Port autoselected: %1$s.", serialPortName);
			...
			if (getLogger() != null)
				getLogger().trace("Creating serial port '%1$s'...", serialPortName);
			...
			if (modelOptions == null) {
				throw new ArduinoUploaderException(
						String.format("Unable to find configuration for '%1$s'!", model)
				);
			}
			...
			default:
				throw new ArduinoUploaderException(
						String.format("Unrecognized MCU: '%1$s'!", modelOptions.getMcu())
				);
			}
